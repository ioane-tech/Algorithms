...
For example, given the list [1, 2, 3, 4, 5], we could choose 2 and 3 for the first operation,
 ...
 is to find the highest possible penalty for a given input.
...
function getTotalTime(arr) {
    ...
    for(var i = 0; i < loopLength; i++){
        if(arr.length != 1){
            ...
            let curentPenalty= arr[arr.length-1] + arr[arr.length-2]
            answer +=  curentPenalty
            
            arr.splice(-2,2,curentPenalty)
  ...
  console.log(getTotalTime([2,3,9,8,4]))